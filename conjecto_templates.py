...
async def chatbot(q: Q):
    ...
    # TODO: Inserção do modelo
    q.page['chatbot'].data += [gpt.query(q.args.chatbot).content, False]
...
async def chat_page(q: Q):
    ...
    add_card(q, 'main-section', ui.form_card(box='vertical', items=[
        ...
        ui.text('<center>Faça uma pergunta abaixo.</center>'),
...
async def upload_page(q: Q):
    ...
    add_card(q, 'form', ui.form_card(box='vertical', items=[
        ...
        ui.table(
            ...
            rows=[ui.table_row(name=f'row{i}', cells=[file.name]) for i, file in enumerate(gpt.get_documments())]
        )
    ]))
...
async def file_upload(q: Q):
    ...
    for f in q.args.file_upload:
        gpt.upload_file(f.split('f/')[1].replace('/','\\'))
...
async def init(q: Q) -> None:
    ...
    q.page['sidebar'] = ui.nav_card(
        box='sidebar', color='primary', title='Conjecto Templates', subtitle="www.conjecto.ai",
        value=f'#{q.args["#"]}' if q.args['#'] else '#chat',
        image='https://conjecto.com.br/wp-content/uploads/2020/11/logo.png', items=[
